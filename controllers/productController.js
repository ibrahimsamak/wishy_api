...
exports.getHome = async (req, reply) => {
  try {
    ...
    for await (const data of advs) {
      var _ad = {
        _id: data._id,
        name: data[`${language}Title`],
        description: data[`${language}Description`],
        image: data.image ? data.image : "",
        is_ads_redirect_to_store: data.is_ads_redirect_to_store
          ? data.is_ads_redirect_to_store
          : false,
        adv_for: data.adv_for ? data.adv_for : 1,
    ...
    for await (const _item of categories) {
      var item = await Product.find({
        $and: [
          { _id: { $in: products_ids } },
          { category_id: _item._id },
          { isNewProduct: true },
          { isDeleted: false },
        ],
      })
      ...
      var rootCateogory = {
        _id: _item._id,
        name: _item[`${language}Name`],
        image: _item.image ? _item.image : "",
        products: returnArr,
      };

      results.push(rootCateogory);
    }
...
exports.getAllProductPlaceByAdmin = async (req, reply) => {
  try {
    ...
    var productsResult = [];
    if (String(req.body.name) != "") {
      productsResult = await Product.find({
        $or: [
          { arName: { $regex: new RegExp(req.body.name, "i") } },
          { arDescription: { $regex: new RegExp(req.body.name, "i") } },
          { enName: { $regex: new RegExp(req.body.name, "i") } },
      ...
      query.$and.push({
        product_id: { $in: productsResult.map((x) => x._id) },
      });
    }
...
exports.addProductPlace = async (req, reply) => {
  try {
    let rs = new Product_Price({
      ...
      price_for_new: req.body.price_for_new,
      price_for_replacment: req.body.price_for_replacment,
      createat: getCurrentDateTime(),
      discountPrice: req.body.discountPrice,
      discountPriceReplacment: req.body.discountPriceReplacment,
      deliveryCost: req.body.deliveryCost,
      expressCost: req.body.expressCost,
      isDeleted: false,
    });
    var _return = handleError(rs.validateSync());
    if (_return.length > 0) {
